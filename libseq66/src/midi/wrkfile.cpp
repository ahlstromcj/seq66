...
static const std::string CakewalkHeader("CAKEWALK");
...
bool
wrkfile::parse (performer & p, int screenset, bool importing)
{
...
/**
 *  An override of the midifile function.  All it does is set the m_track_time
 *  value to 0, so far.
 */

sequence *
wrkfile::create_sequence (performer & p)
{
    sequence * result = midifile::create_sequence(p);
 ...
 *
 * \param p
 *      The performer object to perhaps modify by the finalizing of this
 *      sequence.
 *
 * \param seq
 ...
 *
 * \param seqnum
 *      The prospective sequence number, passed to midifile ::
 *      finalize_sequence().
 *
 * \param screenset
 *      The prospective screen-set number, passed to midifile ::
 *      finalize_sequence().
 */
...
{
    bool result = midifile::finalize_sequence(p, seq, seqnum, screenset);
    if (result)
    {
        ++m_track_count;
        ++m_seq_number;
 ...
 *              to be used during the Seq66 session.
...
wrkfile::TrackChunk ()
{
    ...
    bool loop = ((flags & 4) != 0);         /* always true in Seq66       */
...
)
{
    if (trackno != m_track_number)
    {
        ...
        m_current_seq = create_sequence(*m_performer);
...
wrkfile::NoteArray (int track, int events)
{
    ...
    for (int i = 0; i < events; ++i)
    {
        ...
        else if (status == 5)               /* not supported in Seq66     */
        {
        ...
        else if (status == 6)               /* not supported in Seq66     */
        {
        ...
        else if (status == 7)               /* not supported in Seq66     */
        {
            len = read_32_bit();
            text = read_string(len);
...
/**
 *  Seq66 currently doesn't handle variable time signatures.  So we set it
...
wrkfile::TempoChunk (int factor)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        if (m_track_number == 0)
        {
            m_performer->set_beats_per_minute(bpm);
            m_performer->us_per_quarter_note(int(tt));
 ...
 *  a WRK file in the Seq66 library.
 */
