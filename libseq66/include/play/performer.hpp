...
public:
    ...
    bool set_track_info (const std::string & s, seq::number trk = 0);
    event get_track_info_event (seq::number trk, bool nextmatch = false);
    void song_info (const std::string & s, seq::number trk = 0);
    std::string song_info () const;
    std::string get_all_track_text (seq::number trk);
...
public:
    ...
    void record_by_buss (bool flag)
    {
        ...
        if (master_bus())
            master_bus()->record_by_buss(flag);
    }
...
public:
    ...
    std::string duration (timeformat dur = timeformat::bbt) const;
    ...
    bool send_macro (const std::string & name)
    {
    ...
    {
        return midi_control_out().send_macro(byts);
    }

    bool send_macro_bytes
    (
        const midimacro & mac,
        int b = int(null_buss()),
        int ch = int(null_channel())
    );
    ...
    const midimacro & get_macro (const std::string & name) const
    {
        return midi_control_out().macro(name);
    }
...
private:
    ...
    void playlist_auto_play (bool on);
