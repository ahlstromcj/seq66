 ...
 *  midi_jack_data::jack_port() here, it should be good, or else.
 *
...
jack_process_rtmidi_input (jack_nframes_t framect, void * arg)
{
    ...
    rtmidi_in_data * rtindata = jackdata->jack_rtmidiin();
    void * buf = ::jack_port_get_buffer(jackdata->jack_port(), framect);
    ...
    for (int j = 0; j < evcount; ++j)
    {
        ...
        {
            jack_time_t jtime = ::jack_get_time();  /* time in microsec (!) */
            ...
            else
            {
                ...
                delta_jtime = jack_time_t(jtime * 0.000001);    /* secs???  */
            }
...
#if defined USE_FRAME_OFFSET_PEEKING
 ...
 *      of message size, plus the message itself.
 *
 * \param framect
 ...
 *
 * \param lastoffset
 *      Provides the calculated offset of the previous message.  If the
 *      calculated offset is less than this value, this means that the
 *      event should be put off until the next process callback call.
 *
 * \param [out] dest
...
jack_process_rtmidi_output (jack_nframes_t framect, void * arg)
{
    ...
    /*
     * This frame value is usually a large integer:
     *
     * jack_nframes_t frame = jack_last_frame_time(client);
     */
    ...
    for (;;)
    {
        ...
        (
            buffmsg, framect, last_offset, mbuf, destsz
        );
        ...
        const midibyte * mbytes = reinterpret_cast<const midibyte *>(mbuf);
        short space = 0;
        const size_t sz = sizeof(space);
        char * sp = reinterpret_cast<char *>(&space);
        size_t tssize = midi_message::size_of_timestamp();
	    ...
	    bool process = ::jack_ringbuffer_peek(buffmsg, sp, sz) == sz;
        if (process)
            msgsz = ::jack_ringbuffer_read_space(buffmsg);
        if (msgsz > 0)
        {
            jack_nframes_t offset;
            int ct = ::jack_ringbuffer_read(buffmsg, sp, sz);
            const jack_midi_data_t * data =
                reinterpret_cast<const jack_midi_data_t *>(mbuf);

            ct = ::jack_ringbuffer_read(buffmsg, mbuf, size_t(space));

            midipulse ts = midi_message::extract_timestamp(mbytes, ct);
            data += tssize;
            space -= tssize;
            offset = midi_jack_data::jack_frame_offset(framect, ts);    // 0
            if (offset < last_offset)
                break;
            else
                last_offset = offset;

            int rc = ::jack_midi_event_write(buf, offset, data, size_t(space));
            if (rc != 0)
            {
                async_safe_errprint("JACK MIDI buffer overflow");
                ::jack_midi_clear_buffer(buf);  // or "break'?
...
midi_jack::~midi_jack ()
{
/// if (not_nullptr(jack_data().jack_buffsize()))
///     ::jack_ringbuffer_free(jack_data().jack_buffsize());
...
{
#if defined SEQ66_PLATFORM_DEBUG_TMI
    jack_client_t * client = jack_data().jack_client();
 ...
 *  the message itself to the JACK ring buffer.  The message size consists
 *  of the pulse timestamp and the status+data bytes. The timestamp was not
 *  provided by the original "rtmidi" implementation.  This new data is
 *  added for issue #100.
 *
 * \param message
 *      Provides the MIDI message object, which contains the bytes to send.
 *      Note that this function supports only up to 32565 bytes.
 *
 * \return
 ...
 *
 * \param e24
 *      Provides a pointer to the event.  Currently we do not use the normal
 *      event time-stamp, which is in units of ticks, but instead insert
 *      the current JACK time in microseconds.
 */
...
midi_jack::api_continue_from (midipulse tick, midipulse /*beats*/)
{
    ...
    send_byte(tick, EVENT_MIDI_CONTINUE);
    api_flush();                                /* currently does nothing   */
    send_byte(tick, EVENT_MIDI_SONG_POS);
}
...
midi_jack::api_start ()
{
    ...
    send_byte(0, EVENT_MIDI_START);             /* is tick 0 always good    */
}
...
midi_jack::api_stop ()
{
    ...
    send_byte(0, EVENT_MIDI_STOP);              /* do we need real tick?    */
}
...
#endif
        send_byte(tick, EVENT_MIDI_CLOCK);
    }
}
...
void
midi_jack::send_byte (midipulse tick, midibyte evbyte)
{
    midi_message message(tick);
    message.push(evbyte);
    if (jack_data().valid_buffer())
    {
        if (! send_message(message))
    ...
    m_client_name   ()
{
    ...
    jack_data().jack_rtmidiin(input_data());
}
...
{
    rtmidi_in_data * rtindata = jack_data().jack_rtmidiin();
