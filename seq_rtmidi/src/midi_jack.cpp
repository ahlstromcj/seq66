 ...
 *
 * jack_time_t t = jack_get_time() returns JACK's current system time
 * in microseconds, using the JACK clock source. Guaranteed to be monotonic,
 * but not linear. The jack_time_t type is uint64_t.
 *
 * Question:
 *
...
/**
 *  We should use the async print methods.
 */

static void
message_time (const midi_message & msg, uint64_t t = 0)
{
    const char * tag = t == 0 ? "Sent" : "Delta" ;
    unsigned jtime = unsigned(msg.msg_send_time());
    if (t > 0)
        jtime = t - jtime;
...
)
{
    ...
    if (process)
    {
        ...
        message_time(msg, uint64_t(::jack_get_time()));
#endif
...
jack_process_rtmidi_output (jack_nframes_t framect, void * arg)
{
    ...
    for (;;)
    {
        ...
        if (msgsz >= c_jack_ringbuffer_size)
        {
    ...
    m_jack_data         ()
{
    ...
    jack_data().ticks_per_beat(ppqn() * 10.0);
    jack_data().beats_per_minute(bpm());
}
...
midi_jack::api_play (const event * e24, midibyte channel)
{
...
#if defined SEQ66_SHOW_TIMING
    message.msg_send_time(uint64_t(::jack_get_time()));
#endif
    ...
    if (jack_data().valid_buffer())
    {
        ...
        {
#if defined SEQ66_SHOW_TIMING_DISABLED_HERE
 ...
 *  module.
 *
 *  jack_nframes_t t = jack_frame_time() returns the estimated current time in
 *  frames. This function is intended for use in other threads (not the
 *  process callback). The return value can be compared with the value of
...
midi_jack::create_ringbuffer (size_t rbsize)
{
    ...
    {
#if defined SEQ66_USE_MIDI_MESSAGE_RINGBUFFER
        ring_buffer<midi_message> * rb =
            new (std::nothrow) ring_buffer<midi_message>(rbsize);
...
midi_in_jack::api_poll_for_midi ()
{
    ...
    (void) microsleep(std_sleep_us());                          /* 10 us */
