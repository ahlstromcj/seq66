    ...
    m_timer                 (nullptr)
{
    ...
    else
    {
        ...
        qt_set_icon(wrench_xpm, ui->btn_pattern_fix);
        connect
        (
        ...
        if (doable)
        {
            ...
            if (rational)                       /* use OK'ed it         */
            {
                ...
                else                             /* get_left_tick()      */
                {
                    ...
                    int bpbindex = beats_per_bar_list().index(bpb);
                    int bwindex = beatwidth_list().index(bw);
                    ui->m_combo_bpm->setCurrentIndex(bpbindex);
...
qseqeditframe64::show_lfo_frame ()
{
    ...
    {
        bool ok = track().playable_count() > 0;
        if (ok)
        {
            m_lfo_wnd = new (std::nothrow)
                qlfoframe(perf(), track(), *m_seqdata);
        ...
        else
        {
            ...
            (
                this,
                "No playable events to modulate. "
                "Add notes or controllers first."
            );
        }
    }
    else
