...
%  It is inactive if there is nothing to redo.
   ...
   The record snap button is a bit counter-intuitive.  If disabled,
   it allows only clips of the full pattern to be added.
   If enabled, it turns on record-snap for recording live performance triggers,
   and it also enables the grid snap functionality noted below.
   It also allows the trigger to be smoothly extended without snapping as well.
