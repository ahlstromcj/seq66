...
%  It is inactive if there is nothing to redo.
   ...
   \textsl{L, 1/1, 1/2, 1/4, 1/8, 1/16, and 1/32}

   These fractions represent fractions of measures.
   The \texttt{L} entry, when \textbf{Snap} is on, causes the addition of a
   trigger to act as in \textsl{Seq24}:  the pattern is added, but snaps
   to the length of the pattern, and can result in the trigger beginning well
   before the click.

   Also, in patterns longer than a measure, if \texttt{L} is not checked
   the pattern may wrap, so that beginning note appear at the end of the
   trigger!  When this happens, a trick is needed:

   \begin{enumerate}
      \item Undo that trigger insertion.
      \item Select the \texttt{L} Snap value.
   ...
   This trick is annoying, and we're not sure if it is a feature or a bug.
