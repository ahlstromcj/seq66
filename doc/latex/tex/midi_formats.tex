...
\label{subsec:proprietary_track_format}
   ...
   \index{SeqSpecs}
   The format of sequencer-specific meta-events (\textsl{SeqSpecs} for short)
   is:
   ...
   All of the SeqSpecs are shown in the next table. It shows the name, length,
   and data for each one. A length of 0 means the SeqSpec is not implemented.
   ...
   Some values only appear if they are non-default, such as patterns
   having no color.
   ...
   this SeqSpec if present.
   SeqSpecs named "gap" or "reserved" are not used.
   ...
   \begin{table}[htb]
      ...
      \begin{tabular}{l l l l}
         ...
         \texttt{c\_triggers}        & TBD           & 0                 & \texttt{0x24240004 (see c\_triggers\_new)} \\
         ...
         \texttt{c\_triggers\_new}   & Track         & 4+triggercount*12 & \texttt{0x24240008 triggers...} \\
         ...
         \texttt{c\_tempo\_track}    & TBD           & 8                 & \texttt{0x2424001A track:L} \\
         \texttt{c\_seq\_color}      & Track         & 5                 & \texttt{0x2424001B color} \\
         \texttt{c\_seq\_edit\_mode} & Kepler34      & 0                 & \texttt{0x2424001C} \\
         \texttt{c\_seq\_loopcount}  & TBD           & 6                 & \texttt{0x2424001D 00 00} \\
      \end{tabular}
   \end{table}

   Note that the base length is 4 bytes, the size of a \texttt{0x242400nn}
   value.  Also note that some of multi-byte (2 or 4 bytes) values that indicate counts
   are stored in big-endian (network order) format.  The most-signifant byte is grabbed first, then
...
\label{subsubsec:proprietary_track_seqspec_midibus}

   \begin{description}
      \item \texttt{c\_midibus}: specifies the desired output buss/port number for a track.
      \item \textsl{Length}: 5
      \item \textsl{Format}: \texttt{0x24240001 buss}
   \end{description}

   The buss value ranges from 0 to 47, though the practical range is under a dozen ports.
   Currently in \textsl{Seq66}, buss information is stored by number only.
   However, there is a port-mapping mechanism in the 'rc' file that let's one assign
   permanent port numbers by name, and translate those to actual port numbers.
   See \sectionref{sec:port_mapping}.

\subsubsection{SeqSpec c\_midich}
\label{subsubsec:proprietary_track_seqspec_midich}

   \begin{description}
      \item \texttt{c\_midich}: specifies the desired output channel number for a track.
      \item \textsl{Length}: 5
      \item \textsl{Format}: \texttt{0x24240002 channel}
   \end{description}

   This channel value ranges from 0 to 15, and has an "null" value of 0x80.  If the channel
   ranges from 0 to 15, that channel is applied to every channel event that goes
   out from that sequence.  Otherwise, the channel held by an event is used for output.
   Note that SMF 0 MIDI files have a single track and can have a mixture of different
   channels in its channel event. Also note that support for 0x80 is new and still being
   worked out.

\subsubsection{SeqSpec c\_midiclocks}
\label{subsubsec:proprietary_track_seqspec_midiclocks}
   ...
   Clocking is specified in the 'rc' file.
   See \sectionref{subsec:configuration_rc_midi_clock}.

\subsubsection{SeqSpec c\_triggers}
\label{subsubsec:proprietary_track_seqspec_triggers}

   \begin{description}
      \item \texttt{c\_triggers}: specifies the old format for triggers.
      \item \textsl{Length}: Indeterminate
      \item \textsl{Format}: \texttt{0x24240004 buss}
   \end{description}

   This SeqSpec is no longer used, and is flagged as an error if encountered.
   Instead, \texttt{c\_triggers\_new} is used.

\subsubsection{SeqSpec c\_notes}
\label{subsubsec:proprietary_track_seqspec_notes}

   \begin{description}
      \item \texttt{c\_notes}: specifies the names of the sets in the tune.
      \item \textsl{Length}: 2 + variable
      \item \textsl{Format}: \texttt{0x24240005 setcount strings...}
   \end{description}
...
\label{subsubsec:proprietary_track_seqspec_timesig}
   ...
   This SeqSpec specifies the time-signature for the track in a 2-byte format,
   the beats-per-bar followed by the beat-width.
...
\label{subsubsec:proprietary_track_seqspec_bpmtag}
   ...
   value is the value of the BPM multiplied by 1000.0 (see the
   macro SEQ66\_BPM\_SCALE\_FACTOR).  When read, it is divided by 1000 to get the
   ...
   The function \texttt{tempo\_us\_from\_bytes()} calculates the microseconds from
   these three bytes; the "inverse" function is
   \texttt{tempo\_us\_to\_bytes()}.

   Generally, the MIDI tempo comes first in the file, and the SeqSpec tempo comes
   later.  The last value obtain is the BPM that the performer module contains.
   The conversion between the SeqSpec format and the MIDI Tempo format is
   effected by the functions
   \texttt{bpm\_from\_tempo\_us()} and \texttt{tempo\_us\_from\_bpm()}.

\subsubsection{SeqSpec c\_triggers\_new}
\label{subsubsec:proprietary_track_seqspec_triggers_new}

   \begin{description}
      \item \texttt{c\_triggers\_new}: specifies the triggers for a given track.
      \item \textsl{Length}: 4 + triggercount * 12
      \item \textsl{Format}: \texttt{0x24240008 buss}
   \end{description}
   ...
   The format of each trigger is three 4-byte values: \texttt{on off offset}.
...
\subsubsection{SeqSpec c\_mutegrops}
\label{subsubsec:proprietary_track_seqspec_mutegrops}

   \begin{description}
      \item \texttt{c\_mutegrops}: specifies the mute-groups in play in a song.
   ...
   size 32 patterns.  More flexibility is planned and partially support now.
...
\subsubsection{SeqSpec c\_gap\_ABCDEF}
\label{subsubsec:proprietary_track_seqspec_gap_abcdef}

   This set of six SeqSpecs is a gap created by skipping from
   \texttt{0x24240009} to \texttt{0x24240010}, a long-standing
   oversight from \textsl{Seq24}.
   We guarantee that \textsl{Seq66} will never use these values.

\subsubsection{SeqSpec c\_midictrl}
\label{subsubsec:proprietary_track_seqspec_midictrl}
   ...
   save them in the 'ctrl' file.  This SeqSpec is parsed if present, but
   the data is thrown away, and this SeqSpec is never written.
...
\label{subsubsec:proprietary_track_seqspec_musickey}
...
\label{subsubsec:proprietary_track_seqspec_musicscale}
...
\label{subsubsec:proprietary_track_seqspec_backsequence}

   \begin{description}
      \item \texttt{c\_backsequence}: specifies the background sequence to be shown.
      \item \textsl{Length}: 8
...
\label{subsubsec:proprietary_track_seqspec_transpose}

   \begin{description}
      \item \texttt{c\_transpose}: specifies if a pattern can be transposed.
      \item \textsl{Length}: 5
      \item \textsl{Format}: \texttt{0x24240014 buss}
   \end{description}

   This SeqSpec applies to patterns only.
   It is set to a non-zero value to indicate
...
\label{subsubsec:proprietary_track_seqspec_perf_bp_mes}
   ...
   Provides an override for the beats-per-bar from the Time Signature in track 0.

\subsubsection{SeqSpec c\_perf\_bw}
\label{subsubsec:proprietary_track_seqspec_perf_bw}
...
\label{subsubsec:proprietary_track_seqspec_tempo_map}
...
\subsubsection{SeqSpec c\_reserved\_1\_2}
\label{subsubsec:proprietary_track_seqspec_reserved_1_2}
...
\label{subsubsec:proprietary_track_seqspec_tempo_track}
...
\label{subsubsec:proprietary_track_seqspec_seq_color}
...
\label{subsubsec:proprietary_track_seqspec_seq_edit_mode}

   \begin{description}
      \item \texttt{c\_seq\_edit\_mode}: specifies the editing mode of each pattern.
      \item \textsl{Length}: Indeterminate
      \item \textsl{Format}: \texttt{0x2424001C}
   \end{description}

   This looks like a way to specify that a pattern should be edited in drum mote (1) or
   normal mode (0).  This is a \textsl{Kepler34} feature. However, in \textsl{Seq66} it
   is currently not implemented.  We don't even read it.

\subsubsection{SeqSpec c\_seq\_loopcount}
\label{subsubsec:proprietary_track_seqspec_seq_loopcount}

   \begin{description}
      \item \texttt{c\_seq\_loopcount}: specifies the number of times a pattern should play.
      \item \textsl{Length}: 6
      \item \textsl{Format}: \texttt{0x2424001D count:S}
   \end{description}

   This item is a future feature to provide a user's request for a one-shot pattern,
   a pattern that plays once and never plays again.  A count value of 0 would yield
   ...
   However, a loop-count is used in the one-shot step-edit feature used in recording
   a device's drum pattern.
