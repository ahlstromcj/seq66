...
\end{figure}
   ...
   \begin{enumber}
      ...
      \item \textbf{Rereate! (maps)}
      \item \textbf{Re-enable (maps)}
...
%  If changed, the \textbf{Restart Seq66!} button is enabled.
   ...
   \itempar{Recreate! (maps)}{midi i/o!port mapping}
   \index{port mapping}
   \index{port!mapping}
   ...
   \itempar{Re-enable! (maps)}{midi i/o!re-enable mapping}
   \index{port re-enable mapping}
   \index{port!re-enable mapping}
   This button assumes that the user knows that formerly missing
   devices have been plugged back in, so they need to be
   re-enables. After restarting, the changes in the clock and input maps
   are saved in the 'rc' file.
   This button is needed because \textsl{Seq66} does not
   automatically "rewrite" the ports in the 'rc' file at startup.
   This is a juggling act we need to master at some point.
   ...
   \index{MIDI Thru!warning}
   \textsl{Do not use the ALSA MIDI Through ports for both control
   and display. This creates a MIDI feedback loop.
   Use an actual device port.}
   ...
   If devices that are present keep showing up as disabled, one
   might have to manually edit the 'rc' file to change, in the
   MIDI Clock sections, the "-1" values to zero.
   We gotta fix that!
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_input_tab-2.png}
...
\end{figure}
   ...
   \index{MIDI Thru!warning}
   \textsl{Do not use the ALSA MIDI Through ports for both control
   and display.  Use an actual device port.}
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_input_tab-virtual.png}
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_display_tab.png}
...
\end{figure}
   ...
   Note that this option will not work if a
   \textsl{Seq66} play-list is active.
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_jack_tab.png}
...
\end{figure}
   ...
   \begin{itemize}
      ...
      \item \textbf{JACK Auto-Connect}.
         ...
         It can be toggled off, to let the user
         or a session manager make the connections.
   \end{itemize}
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_play_options_tab.png}
...
\end{figure}
   ...
   \itempar{Automatically add starting time-signature event}{edit!timesig}
   If checked, a time-signature event is added to the beginning of a
   track if none is present.
   ...
   from 24 to 19200.
   960 PPQN is probably the highest useful PPQN.
   When this option is changed, the \texttt{-{}-user-save} option is turned on
   to preserve the setting when \textsl{Seq66} exits.
   ...
   \itempar{Song Start Mode}{edit!song-start mode}
   This item determines the mode for playback.

   \begin{enumerate}
      \item \textbf{Live}.
         The arming and muting of patterns is controlled by the musician,
         by clicking the live grid's button, using the grid slot
         hot keys, or a MIDI controller configured for this purpose.
      \item \textbf{Song}.
         The arming and muting of patterns is controlled by the
         triggers layed out in the song editor.
      \item \textbf{Auto}.
         The Song mode is set if the MIDI tune contains any triggers;
         otherwise the Live mode is set.
   \end{enumerate}

   The current mode is shown on a button at the bottom of the main window.
   This button can be clicked to change the mode.

\subsection{Edit / Preferences / Metronome Options}
\label{subsection:edit_preferences_metronome}

   \index{recording!background}
   \index{recording!scratchpad}
   This tab contains options for the "metronome" and
   "background recording" features.
   However, note that "background recording" in
   \textsl{Seq66} is different from other applications,
   so we called "scratchpad recording".
   A "scratchpad" pattern is created, playback commences, and recording
   can progress unhindered.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_metro_options_tab.png}
...
\end{figure}
   ...
   button.
   The metronome is a standard \textsl{Seq66} pattern used
   ...
   song.
   The settings shown above are saved to a
   \texttt{[metronome]} section in the 'rc' file.
   ...
   \itempar{Set Defaults}{metronome!defaults}
   This sets the metronome itself to use the internal
   hard-wired defaults for playback.
   It sets the metronome to use channel 10, usually used
   for drums.
   In \textsl{FluidSynth}, for example, a low stick and high stick
   are used for the measure beat and the other beats.
   ...
   background/scratchpad record notes.  Otherwise they will not be heard
   (unless the device produces sound).
   It \textsl{must} be enabled in the \textbf{MIDI Clock} list.
   If this port is set for usage, then the created "scratchpad"
   pattern is automatically armed, so the notes can be heard.
...
%  Note that the background recorder only works when playing, which
%  kind of defeats the purpose at this time.
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_pattern_tab.png}
...
\end{figure}
   ...
   \begin{itemize}
      ...
      \item \textbf{Apply only to new tracks}.
   ...
   \textbf{Randomization}.
   These items set the ranges for jittering the events in time or
   randomizing their amplitude.
...
\label{subsection:edit_preferences_session}
   ...
   This tab contains options related to normal session management
   (that is, running without JACK Session or the Non/New Session
   Manager) and the setup of the configuration files.
   Also see \sectionref{subsubsec:sessions_tab}, which shows
   more details.
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/edit/preferences/midi_session_tab.png}
...
%  currently an \textsl{experimental} and \textsl{untested} option.
   ...
   Note that activating a play-list file will deactivate the
   load-most-recent file option.
