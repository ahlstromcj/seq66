...
\label{subsubsec:introduction_ppqn_selection}
   ...
   \texttt{24, 32, 48, 96, 120, 192, 240, 384, 480, 768, 960, 1920, 2400, 3840,
   7680, 9600, and 19200}.
   The PPQN cannot be changed to values not in this list, but
   if a file with an unsupport PPQN is loaded, that will show up
...
%  (Modify the file to use a supported value, if possible.)
   If a MIDI file is loaded, this setting can scale that file,
   rescaling all the pattern events and pattern triggers to
   the base PPQN (set to 192 by default, but modifiable in 
   \textbf{Preferences / Play / PPQN}), without
   raising the "modified" flag, if specified
   Also see \sectionref{subsection:edit_preferences_play_options}.

   Generally, MIDI recommends PPQN that are units of 24 ticks.
   The value 32 is included anyway, but, if selected, the
   displays of notes and grids are incorrect.
   We don't intend to fix this.
   There are some display issues at 2400, 9600, and 19200 PPQN,
   which we do intend to fix.
