...
\label{subsec:alsa_through_ports}
   ...
   Notice that there is only one Through port.
   ...
   "Midi Through" pseudo-device, 0. Note the spelling, "Midi".
   ...
   Also see how to increase the number of ALSA MIDI Through ports in
   \cite{alsathru}.
   ...
   \index{MIDI Thru}
   \index{ALSA Through}
   One minor issue with the system's ALSA MIDI Through port is that, if it and
   another input port (e.g. VMPK) are both enabled, then each note emitted by
   VMPK is doubled. Be aware.
   ...
   This problem also occurs if one has set up an active 'ctrl'
   using the MIDI Through ports for control \textsl{and}
   display, \textsl{and} has set a non-empty \texttt{startup} macro.
   The macro goes out the display port and is received on the control
   port, and chaos ensues, including a likely application crash.
   A warning pops up if both in and out are set to a "Midi Through"
   port.
   If not changed, at the next startup this situation is detected and the
   control/display sections of the 'ctrl' file are disabled.

   \index{MIDI Thru!warning}
   \index{ALSA MIDI Through!warning}
   \textsl{Never use the ALSA MIDI Through port for both control
   ...
   To add more ALSA Through ports, first use \texttt{modinfo} to see
   information about the kernel module \texttt{snd-seq-dummy}.  Part of the
   ...
   \textbf{Note}: We use the '\#' prompt to indicate root access or
   usage of \texttt{sudo}.
...
\label{subsec:alsa_virtual_midi_devices}
   ...
   One instance of \textsl{Seq66} can communicate
   another instance.
   See \sectionref{paragraph:alsa_testing_midi_clock_receive};

\label{paragraph:alsa_testing_midi_clock_receive}

\subsubsection{ALSA / Virtual MIDI Devices / snd\_virmidi}
\label{subsubsec:alsa_virtual_midi_devices_snd_virmidi}

   The \texttt{snd\_virmidi} ("virmidi" for short)
   Linux kernel module creates ALSA fully functional
   "hardware" virtual ports that can be used
   to route MIDI messages between software clients.

   \textbf{Note}: To use Seq66's virtual/manual MIDI ports, the "virmidi"
   modules need not be present. 

   \begin{verbatim}
      $ lsmod | grep virmidi
   \end{verbatim}

   Both \texttt{snd\_virmidi} and \texttt{snd\_seq\_virmidi}
   should be present.
   If not, then they must be added.
   Load the modules temporarily using

   \begin{verbatim}
      # modprobe snd-virmidi                 (defaults to 4 "devices")
      # modprobe snd-virmidi midi_devs=1     (one "device")
   \end{verbatim}

   Run the second command so only one virtual port is made.
   Run \texttt{lsmod} as above to verify the "virmidi" modules.
   If present run these commands for more verification:

   \begin{verbatim}
      $ cat /proc/asound/cards
       0 [PCH     ]: HDA-Intel - HDA Intel PCH
                     HDA Intel PCH at 0xa1228000 irq 141
   ...
   \begin{verbatim}
      $ aconnect -i    ---or---     aconnect -o
      client 44: 'Virtual Raw MIDI 7-0' [type=kernel,card=7]
          0 'VirMIDI 7-0     '
   \end{verbatim}

   One will see something like the above, as well as other ALSA
   clients.

   Load the module permanently at boot by adding
   \texttt{snd\_virmidi} to:
   \texttt{/etc/modules-load.d/modules.conf}
   or by copying \texttt{contrib/scripts/virmidi.conf}
   to 
   \texttt{/etc/modules-load.d}.

%  Set a fixed card index by creating a file in
%  /etc/modprobe.d/snd-virmidi.conf with 
...
%  \end{verbatim}
   ...
   is \texttt{aconnectgui}. The command-line version is
   \texttt{aconnect}.
   Also see the article about ALSA and JACK MIDI \cite{midilinux}.

   At present, we have no use-case to describe
   for these virtual ports.
   One use is for connecting more than one port to another port
   using \texttt{aconnect}.
...
\label{paragraph:alsa_testing_midi_clock_receive}
   ...
   \begin{verbatim}
      $ qseq66 --alsa --manual --verbose --client-name seq66debug
   \end{verbatim}

   In
   \textbf{Edit / Preferences / MIDI Input}, check-mark the first
   "seq66" port.  There should be no need to restart.
