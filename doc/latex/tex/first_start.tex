...
\label{sec:introduction_lets_go}
   ...
   Note that the use of a log file can be disabled.
   ...
   Note that \texttt{/usr/} or \texttt{/usr/local/}
   is where the program and its data files
   are stored, and \texttt{-0.99} is the main version number.
   ...
   \begin{verbatim}
      /usr/local/share/seq66-0.99/                       (Linux)
...
\label{subsec:introduction_device_changes}
   ...
   click the \textbf{Recreate} button.
   This action stores (again) the current set of MIDI devices.
   Or, if some devices were missing (and thus flagged as unavailable
   in the 'rc' file), and are now availabled, click the
   \textbf{Re-enable} button.
   ...
   (We need a dialog, at some point, to make this easier.)
...
\label{subsec:introduction_ports}
   ...
   port numbers in the MIDI file to the numbers of existing ports,
   and then save the MIDI file.
