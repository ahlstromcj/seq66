...
\label{sec:sessions}
   ...
   \begin{enumber}
      \item \textbf{Signals}.
         During a conventional run, \textsl{Seq66} will respond
      ...
      \item \textbf{JACK Session}
         ...
         be saved.
         There are no restrictions on where the MIDI files can be stored.
   ...
   For true session management, \textsl{NSM} is the way to go.
...
% TODO: add cites for the above.
   ...
   will detect it and will set up for NSM usage.
...
\subsection{Session Management / NSM}
...
\subsubsection{Session Management / NSM / Run in NSM}
...
\subsubsection{Session Management / NSM / File Menu}
...
\end{figure}
   ...
   This has been changed; the \textbf{Quit} menu entry becomes
   \textbf{Hide}, as per the NSM protocol.
   Also have fixed a bug that disables
...
\subsubsection{Session Management / NSM / Debugging}
...
\subsection{Session Management / LASH}
...
\subsection{Session Management / Sessions Tab}
\label{subsec:sessions_tab}

   The \textsl{Session} tab is a mostly \textsl{read-only} tab
   provided to orient the user to the setup supported by the session.
   When not running in a JACK or NSM session, the normal configuration
   directory and files are shown.
   Also see the Session tab in
   \sectionref{subsection:edit_preferences_session}.

   When running in an \textsl{NSM} section, the configuration
...
%  as non-functional.
   ...
   Other than status display, there a also some changes and action
   that can be performed, as described below.
...
%  \includegraphics[scale=0.65]{tabs/session/qseq66-session-tab.png}
   \includegraphics[scale=0.65]{tabs/session/main-session-tab.png}
   \caption*{Session Tab}
\end{figure}

\subsubsection{Session Management / Sessions Tab / Interface Items}
\label{subsubsec:sessions_tab}
   ...
   \begin{itemize}
      ...
      \item \textbf{Macros}.
         ...
         Select one, then click the \textbf{Send} button.
         The macro bytes are sent out the
      ...
      \item \textbf{Ctrl Out}.
         The drop-down contains this item as the first, and default one.
         It specifies the MIDI control out port.
         After this item, all of the available output ports are 
         shown. This makes it easy to direct a macro to another
         device.
      \item \textbf{Send}.
         This button sends the selected macro to the selected bus.
      \item \textbf{Delete}.
         This button deletes the selected macro and marks the 'ctrl'
         file for saving at exit.
      \item \textbf{Session Log File}.
         ...
         If cleared by the clicking the \textbf{Clear} button,
         then clicking \textbf{Restart},
         the text is shown in the console window (if running in a
         console window). This name is only a base-name (e.g.
         \texttt{qseq66.log}); it is always stored in the "home" configuration
...
\subsubsection{Session Management / Sessions Tab / Notes About Macros}

   First, there are a handful of hardwired macros.
   (See \sectionref{subsubsec:configuration_ctrl_macro_control_out}).
   Additional macros can currently be added in two ways:

   \begin{itemize}
      \item \textbf{Editing the 'ctrl' file}.
         This method involves back up the old 'ctrl' file, then
         adding the desired macro (names and bytes) to a line in the
         \texttt{[macro-control-out]} section.
         Of course, macros can also be deleted from this section,
         or modified.
         See \sectionref{subsubsec:configuration_ctrl_macro_control_out}.
      \item \textbf{The (N)RPN dialog}.
         For adding the RPN or NRPN controller, this dialog is handy,
         as it can add up to six controller events at once, which would
         be tedious to do in the event event editor.
         Macros can be created there.
         See \sectionref{subsec:pattern_editor_rpn_nrpn_dialog}.
   \end{itemize}

   The (N)RPN dialog includes an initial user-interface to make
   macro creation, editing, and deletion more foolproof.

\subsection{Session Management / sessions.rc}
