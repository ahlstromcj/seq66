...
\label{sec:sessions}
   ...
   \begin{enumber}
      ...
      \item \textbf{JACK Session} (not ready, deprecated but will be
         implemented)
      \item \textbf{Signals}
      \item \textbf{LASH} (not ready, may go away completely)
   \end{enumber}

   \textbf{Non Session Manager} provides a replacement for the
   \textsl{JACK Session API} (still enabled in a \textsl{Seq66}
...
qseq66 --jack-midi --jack-slave --jack-session-uuid 84670 --home ${SESSION_DIR}
   ...
   Currently buggy. It thinks the UUID is a file, and sets "home" to
   the \texttt{qtest/seq66master}
   or \texttt{qtest/seq66slave} directory.
   Also, it does not spawn \textsl{QSynth}. (It sounds like that application
   does not support \textsl{JACK Session}.
   We'll try with \textsl{Yoshimi}, which does support it.
