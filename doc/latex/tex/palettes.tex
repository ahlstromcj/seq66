...
\label{sec:palettes}
   ...
   The palette file is somewhat self documenting and easy to edit.
...
\label{subsec:palettes_setup}
   ...
   something like \texttt{qseq66.palette}, plus these sections:
   ...
   The "hints" section helps adapt to dark themes and dark palettes:
   ...
   The fourth section is the "Pattern" palette; the second section is the
...
\label{subsubsec:palettes_theming_qt5ct}
   ...
   If not, use one of the following settings:
   ...
   can take about 20 to 30 seconds to start up on some computer setups!
   Another issue is that some Qt themes might upset the sizing of buttons or
   text.
   Also see the style-sheet discussion in
   \sectionref{sec:configuration}.

   Some theming engines provide small configuration applications that
   can be used to tweak a given theme.
   Two of them are
   \texttt{breeze-settings} and \texttt{kvantummanager}.
