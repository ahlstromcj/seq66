...
\title{Seq66 User Manual v. 0.99.28}
\author{Chris Ahlstrom \\
...
\label{subsec:what_is_seq66}
   ...
   MIDI Learn, macro editing, karaoke MIDI files,
   a modifiable color and brush palette,
   and Qt style-sheets.
   Be prepared to note some significant advances
   in  \textsl{Seq66} from our first reboot, \textsl{Sequencer64}.
...
\label{subsec:introduction_vs_others}
   ...
   code by using newer features of \textsl{C++17};
...
\label{subsec:improvements}
   ...
   \begin{itemize}
      \item \textbf{Meson} is now used as the build system.
         Amazingly fast!
      \item \textbf{Qt} 5 (and 6)
         as the standard user-interface offers many benefits:
         \begin{itemize}
...
\label{subsec:introduction_document_structure}
   ...
   Also note that the various screenshots in this documents
   reflect various Qt themes used during testing and
   figure generation.
...
\label{subsec:introduction_building_seq66}
   ...
   \begin{itemize}
      \item \textbf{Meson Build and Install}.
         Meson loads any subproject, builds the code, and can install
         it and generate this PDF manual.
         Works with \textsl{Linux}, \textsl{Windows}, and \textsl{FreeBSD}.
      \item \textbf{./work.sh}.
         This script provides a lot of canned options to use
         meson to build multiple versions (Clang, debug, etc.)
      ...
      \item \textbf{Arch Linux}.
         There is an \textsl{AUR} package and some
   ...
   Also see \texttt{data/readme.windows} and \texttt{contrib/git/git.odt}
   for supplemental information.

% First Start

\input{tex/first_start}
...
\input{tex/preferences}

% Patterns Panel

\input{tex/patterns_panel}

% Pattern Editor

\input{tex/pattern_editor}

% Song Editor

\input{tex/song_editor}

% Event Editor

\input{tex/event_editor}
...
\input{tex/midi_learn}

% Setup for Launchpad Mini

\input{tex/launchpad_mini}
