...
\end{figure}
   ...
   \itempar{Pattern in window}{pattern!edit in window}
   ...
   \itempar{Pattern in tab}{pattern!edit in tab}
   ...
   \itempar{Events in tab}{pattern!events in tab}
...
\end{figure}
   ...
   \itempar{Flatten triggers}{pattern!flatten}
...
\end{figure}
   ...
   top right changes to show the form "bus:measures".

\begin{figure}[H]
   \centering 
   \includegraphics[scale=1.00]{main-window/slot-with-input-buss.png}
   \caption{Input Bus Notation}
   \label{fig:slot_input_bus_notation}
\end{figure}

   In this figure, the string is "3:1".
...
\label{paragraph:patterns_metronome}
   ...
   This is a feature new with \textsl{Seq66} version 0.99.0,
   and still has a few minor issues to work out, but it works.
