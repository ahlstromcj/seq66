...
\label{sec:patterns_panel}
   ...
   presented in a "slot".
   See \figureref{fig:main_screen_annotated}; see
...
\label{subsec:patterns_panel_main}
   ...
   One can switch between sets by using the square-bracket
   keys to move down and up: "]" to move up by one set, and
   "[" to move down by one set.
...
\label{paragraph:patterns_context_menus}
   ...
   \itempar{Live grid window for set N}{live!external}
...
\end{figure}
   ...
   \itempar{Live grid window for set N}{pattern!external live frame}
...
%  (The loop-record style is not applied at this time. TODO).
   ...
   \index{recording shortcut}
   \index{keys!+}
   \index{keys!record toggle}
...
\end{figure}
   ...
   \textbf{Important}:
   This menu entry is disabled if recording is enabled. Turn off recording for
   the pattern to enable it.
...
\end{figure}
   ...
   \itempar{Track / Flatten triggers}{pattern!flatten}
   This item is shown only if the pattern has song triggers.
   ...
   event; the tune can also be exported to a plain MIDI format.
   This operation is similar to the effect of the \textbf{Export Song}
   function, but for one track.

   \itempar{Track / Export}{pattern!export pattern}
   ...
   \itempar{Track / Copy}{pattern!copy}
   ...
   \itempar{Track / Paste}{pattern!paste}
   ...
   \itempar{Track / Merge}{pattern!merge}
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-window/slot-input-buss-menu.png}
   \caption{Input Bus Popup Menu (old style)}
   \label{fig:slot_input_bus_menu}
\end{figure}
   ...
   \begin{itemize}
      ...
      \item To disable the usage of input-buss routing, set the input buss
         ...
         \sectionref{subsection:edit_preferences_midi_input} and
         restart.
   ...
   If an input buss has been specified, then the measures length at the
   top right changes to show the form "bus:measures". Also shown
   is the asterisk denoting a pattern change.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.85]{main-window/slot-with-input-buss.png}
   \caption{Input Bus Notation}
   \label{fig:slot_input_bus_notation}
\end{figure}

   Note: the input buss setting overrides the record-by-channel
   setting.
...
\label{paragraph:patterns_labels}
   ...
   \begin{itemize}
      ...
      \item \textbf{Pattern Length}. Top right.
         ...
         "forever".
         Lastly, if the is a number followed by a colon, this is
         in input buss number.
      \item \textbf{Notes or Fingerprint}. Center progress box.
         \index{pattern!contents}
         The contents of the pattern, in the central box (or ellipse),
         ...
         \index{empty pattern}
         A pattern with no playable events will not needlessly scroll
         during playback.
   ...
   If the \texttt{Ctrl} key is held during a
   \textsl{left-click} on a pattern,
   a new pattern is created; a prompt comes up if the slot
   is already occupied.
...
\label{paragraph:patterns_pattern_keys}
   ...
   \index{keys!screenset up}
   The "[" key and "]" (square brackets) keys
   on the keyboard decrement or increment the set number.
...
\label{paragraph:patterns_pattern_clicks}
   ...
   dropped.  A pattern \textsl{can} be dragged even if its
   ...
   double-click (if enabled) will.
   If the slot already has a pattern, a prompt will appear.
...
\label{paragraph:patterns_metronome}
   ...
   This feature still has a few minor issues to work out, but it works.
...
\paragraph{Background/Scratchpad Recording}
\label{paragraph:patterns_background_recording}

   \index{background recording}
   \index{scratchpad recording}
   \index{recording!background}
   \index{recording!scratch}
   Also shown in the figure is a \textbf{Scratchpad Recording} button.
   It is not strictly \textbf{background recording}, which involves a
   thread continuously recording in the background all the time;
   here a "scratchpad pattern" is created, ready to record in "expand"
   mode, and playback is started.
   ...
   Count-in recording needs to be enabled in
   \textbf{Edit / Preferences / Metronome}.
   Otherwise, the count-in/background recording button is disabled.
   Once enabled, start the background-recording session:

   \begin{enumber}
      \item Press the background-recording button.
         A new pattern is created in the next available slot, already
         set to record and expand.
      \item Playback starts immediately.
         Nothing is recorded at first.
      \item When ready to improvise, start playing the MIDI instrument.
         All events on the background-recording input buss will be recorded.
         The pattern will continue to expand, basically "forever".
      \item Once one is satisfied with the recording, stop playback.
      \end{enumber}

   The events can be edited, copied, or deleted.
   If the process above is done again, a new background-recording
   pattern is created in the next available slot.

%  Note that the count-in pattern is muted while recording.
%  We will change how it works based on user feedback.

   One might ask why not have background-recording operating all the time,
   in a hidden pattern, as other applications do?

   The the decision to improvise should be a conscious decision.
   The visual feedback of a visible recording slot and the motions of
   progress in the other patterns enhance the improvisation.
   Also, this provides integration with count-in recording.

%  Create an issue if this bothers you.
...
%        for that pattern.
      ...
      \item \textbf{One-shot Reset}.
         ...
         \begin{itemize}
            ...
            \textbf{Track / Clear events}.
            This seems safer, if slightly clumsy.
         \end{itemize}
   \end{itemize}

   These loop-record modes are applied when a pattern is opened
...
\label{subsec:patterns_panel_bottom}
   ...
   The second line of the bottom panel is discussed more completely in
   \sectionref{subsec:introduction_main_bottom_controls_2}
   It is actually part of the main window.
