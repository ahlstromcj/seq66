...
\label{paragraph:patterns_pattern_keys}
   ...
   When the snapshot mode is exited, the
...
%  Lastly, there is only one snapshot key slot.
   Our preference is to use something that does not trigger desktop/window
   ...
   Other patterns can be clicked to be armed; when the "replace" control
...
%  \textsl{Seq66} provides an extension to the replace/solo functionality
   ...
   \textsl{Seq66} intends to provide an extension to the replace functionality
