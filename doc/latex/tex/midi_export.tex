...
\label{subsubsec:midi_export_file_import}
   ...
   into sequences/patterns for each of the 16 MIDI channels.
   Meta events, having no channel, are added to pattern 0.
   The SMF 0 import also puts
   \textsl{all} of the MIDI data into the 17th pattern (pattern 16), in case it
   is needed.  Note that this slot is used no matter which screen-set one
   imports the file into.  Bug, or feature?
   ...
   Finally, automatic conversion to SMF 1 for SMF 0 files can be disabled using
   the 'usr' option \texttt{[user-midi-settings] convert-to-smf-1}.
...
\label{subsec:midi_export_file_export_menu}
   ...
   Also note that exporting a song or an SMF 0 file requires that the song
   be exportable (see the "Export Song" section).
