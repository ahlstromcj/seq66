...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{main-menu/file/light-menu-file-open-2.png}
   \caption{File / Open}
...
\end{figure}
   ...
   pattern, pattern 0 also holds meta events,
   and pattern 16 contains the original track.
...
%  \includegraphics[scale=0.65]{main-menu/file/dark-menu-file-open-playlist.png}
   \includegraphics[scale=0.65]{main-menu/file/menu-file-open-playlist.png}
...
\end{figure}
   ...
   The recent-files list is saved in the \texttt{[recent-files]} section of the
...
%  \includegraphics[scale=0.65]{main-menu/file/dark-menu-file-save-as.png}
   \includegraphics[scale=0.65]{main-menu/file/menu-file-save-as.png}
...
\end{figure}
   ...
   \textsl{Seq66} uses SeqSpecs to encode these two values.
...
\label{subsec:menu_edit}
   ...
   \itempar{Macros}{edit!macros}
   This entry brings up the (N)RPN dialog and automatically sets
   the "Control" selection to "Other" to work with arbitrary data.
...
%  (see \sectionref{sec:pattern_editor}).
   ...
   Move to the desired set (whether empty or not), and then
   ...
   to copy it into another MIDI file.

\subsection{Menu / Learn}
\label{subsec:menu_learn}

   If MIDI Learn has been enabled (and it should be), this menu is available.
   It provides for a rudimentary MIDI Learn feature.
   After reading below, a deeper explanation can be found
   in \sectionref{sec:midi_import_export}.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{main-window/learn-manager-menu.png}
   \caption{Seq66 Learn Menu}
   \label{fig:menu_learn_manager_menu}
\end{figure}
   ...
   The first three of these entries bring up a
   \textbf{Learn Manager} dialog, where
   that entry is the active one.
   The last entry is equivalent to the "L" button used to take a snapshot
   of a mute-group.

   \setcounter{ItemCounter}{0}      % Reset the ItemCounter for this list.

   \itempar{Learn / Loops}{edit!learn loops}
   The \textbf{Learn Manager} comes up with this item selected.
   At this point, the control event for the first loop,
   loop 0, is ready to be recorded.
   The user should then serially click all the desired loop events 
   in the MIDI controller until
   up to 32 loop events are recorded.

   \itempar{Learn / Mute Groups}{edit!learn mute groups}
   The \textbf{Learn Manager} comes up with this item selected.
   It works like the loop learning noted above, but maps the buttons
   for the mute-group settings in a serial fashion.
   The user should serially click all the desired mute-group selection
   events in the MIDI controller until
   up to 32 mute-group selection events are recorded.

   \itempar{Learn / Automation}{edit!learn loops}
   The \textbf{Learn Manager} comes up with this item selected.
   The user has to click the desired action, and then
   click the desired button on the MIDI controller.

   \itempar{Learn / Mute ("L")}{edit!learn mutes}
   This item is the same as "L" button; it stores the current configuration
   of armed patterns.
...
\label{subsec:menu_help}
   ...
   \begin{enumber}
      \item \textbf{About...}
      \item \textbf{Build Info...}
      \item \textbf{Song Summary File...}
      \item \textbf{App Keys...}
   ...
   It has been beefed up with a way to access a
...
\subsubsection{Menu / Help / MIDI Learn...}
\label{subsubsec:menu_help_midi_learn}

   \index{Help!MIDI Learn}
   Contains the following sub-menus:

   \begin{itemize}
      \item \textbf{Tutorial}.
         This entry brings up a short PDF tutorial about MIDI Learn
         using the drum pads on an
         \textsl{Akai MPK Mini Play} controller.
      \item \textbf{Cheat-sheet}.
         This entry brings up a one-page PDF with diagrams that
         show the learnable user-interface items.
      \item \textbf{Quick Reference}.
         This entry brings up a dialog giving a short description of how to use
         the MIDI Learn manager.  It's just enough to jog one's memory after
         reading \sectionref{sec:midi_import_export}.
   \end{itemize}
...
\label{subsubsec:menu_help_tutorial}
   ...
   Windows will have its own methods, but
   ...
   See \sectionref{subsubsec:usr_file_user_options}.
   This setup can also be done in
   \textbf{Edit / Preferences / Session}; see
...
\subsubsection{Menu / Help / View Log...}
\label{subsubsec:menu_help_view_log}

   \index{Help!view log}

   If a log file is in play (as is the default, which can be changed
   in the main window's \textbf{Session} tab), then its contents can
   be viewed in a new window.
