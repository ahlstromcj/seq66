...
\label{subsubsec:menu_edit_preferences}
  ...
  Configuration items not (yet) implemented in \textbf{Preferences} are
   ...
   In the diagrams that follow, the new \textbf{Metronome} tab is not
   shown; but it is discussed in its own section.
...
\label{paragraph:menu_edit_preferences_metronom_options}
   ...
   This new tab contains options for the new "metronome" feature

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{main-menu/edit/preferences/midi_metro_options_tab.png}
...
\end{figure}
   ...
   \itempar{MIDI Buss}{metronome!buss}
   This value select the device to use to play the metronome.

   \itempar{Channel}{metronome!channel}
   This value select the channedevicel to use to play the metronome.

   We still have some more work to do to refine the metronome and its
   configuration.
   Notably, the count-in feature is not ready.
