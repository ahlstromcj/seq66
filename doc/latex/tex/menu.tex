...
\label{subsubsec:menu_file_export}
   ...
   This functionality is explained in detail in
   \sectionref{subsec:midi_export_song_export}.
...
\label{subsubsec:menu_file_export_smf_0}
   ...
   See \sectionref{subsec:midi_export_file_export_smf_0}.
...
\end{figure}
   ...
   (It is difficult to see in the figure with the selected theme. Doh!)
...
\end{figure}
   ...
   \itempar{Input/Output Option}{I/O options}
   ...
   user-interace.  A restart is necessary for this option to take effect.
...
\end{figure}
   ...
   \itempar{Load Most Recent File at Startup}{load most-recent}
   The file at the top of the \texttt{[recent-files]}
   ...
   \itempar{Long Port/Buss Name}{long buss names}
   Controls how much port information is shown in the clocks and input
   listings.  For the "portmidi" implementation, keep this option checked.

   \itempar{Lock Main Window}{lock main window}
...
\end{figure}
   ...
   session.  Also see the \textbf{Session} tab in the main window.
