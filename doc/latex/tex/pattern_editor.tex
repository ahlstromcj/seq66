...
\end{figure}
   ...
   \textbf{Note}:
   In order to keep the time pane (timeline), vertical piano keyboard,
   piano roll, event pane line, and data pane in sync,
   the arrow keys have been disabled
   in all of the items \textsl{except} the piano roll.
   The piano roll must have focus for moving or paging left, right, up, and
   down.

   \index{keys!vi}
   \index{keys!hjkl}
   The exception is that the "h" (left), "j" (down), "k" (up), and "l" (right)
   keys can be used while any pane is in focus, and they act like the arrow
   keys do when the piano roll is in focus..
   These keys function as they do in the "vi" text editor.
...
\end{figure}
   ...
   \begin{enumber}
      ...
      \item The mouse cursor is in the bottom half of the time line, and
         here it's a pointing finger.  When clicked, the "L" marker is set
         there.
      \item The mouse cursor is in the top half of the time line, and is
         shown as a vertical arrow.  It sets the current position in the
...
\label{subsec:pattern_editor_data_view}
   ...
   Tempo events are shown as a small yellow circle.
...
\end{figure}
   ...
   Both of these event values can be modified by dragging a line as discussed
   above.
...
\end{figure}
   ...
   to the events in the data panel.
...
\begin{figure}[H]
   ...
   \caption{Data Pane Grab-Handles}
   \label{fig:pattern_editor_data_pane_grab_handles}
\end{figure}
