...
% \end{figure}
   ...
   from a MIDI file,
%  (whether foreign or native to \textsl{Seq66}),
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.55]{pattern-editor/pattern-edit-second-panel-items.png}
...
%    \caption{Scales Currently Supported in Seq66}
...
\label{paragraph:pattern_editor_note_events}
   ...
   For now, \sectionref{paragraph:menu_file_options_mouse}, such as it
   is, will have to do.
...
%  \textsl{Seq66}.  This issue might depend on the Linux distro one
%  uses.)
%  }
...
%  As an aside, note that the event strip is gray.  \textsl{Seq66}
...
\label{paragraph:pattern_editor_note_events_fruity}
   ...
   For now, see \sectionref{paragraph:menu_file_options_mouse}.
...
%  If \textsl{Seq66} is built with the \texttt{--enable-lfo} option, then
