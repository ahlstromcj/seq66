...
%  $\bullet$
   ...
   1, 1/2, 1/4, 1/8, 1/16 (\textsl{the default value}), 1/32, 1/64, and 1/128.
   ...
   Supported values are 1:1, 1:2 (\textsl{the default value}),
   1:4, 1:8, 1:16, and 1:32, along with
...
%  Also see \sectionref{subsubsec:pattern_editor_zoom_keys}.
   ...
   Changing the \textbf{Key} shifts the marked note-rows
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{pattern-editor/pattern-edit-window-key-numbers.png}
...
%  Pattern or Song editors so that the \texttt{Mod4} key (Super or Windows key)
%  can be pressed when releasing the right mouse button.
%  This keeps the mouse in note-add mode.
%  Another right-click, without pressing \texttt{Mod4}, will exit this mode.
%
...
%  What a crock!
%
%  This option will not interfere with the \texttt{Mod4} key being set
...
%
%  Note editing is a bit different with \textsl{Seq66}, since it
...
%  The \texttt{Mod4}-right-click feature does not
...
\label{subsubsec:pattern_editor_note_editing}
   ...
   Once in note-entry mode, the mouse can be clicked or dragged
   ...
   \begin{itemize}
      \item \index{mouse!left-click} \index{pattern editor!left click}
      ...
      \item \index{mouse!left-click-drag} \index{pattern editor!select multiple notes}
   ...
   \sectionref{subsec:pattern_editor_second} can also be used to
   modify selections.

   Once one or more notes are selected, they can be modified in time,
   pitch, or length.

   \itempar{Move Notes in Pitch}{pattern editor!transpose notes}
...
\label{subsec:pattern_editor_data_view}
   ...
   First, note the buttons at the left.
