...
%  Note the controls in this window:
   ...
   \begin{enumber}
      ...
      \item \textbf{Oneshot}.
         ...
         When this option is set, the record button is on, and no pattern is
         playing, recording won't start until a note comes in, and when the first
         note comes in, the progress bar starts at the left.
         At the end of the pattern, recording stops automatically.
         See the figure below for a recording from a \textsl{Yamaha DD-11}.
         Note that this feature is still experimental, and bugs are being worked
         out.
   \end{enumber}
