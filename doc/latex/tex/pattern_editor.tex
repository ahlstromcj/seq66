...
\label{sec:pattern_editor}
   ...
   In the descriptions that follow, there is a mix of old and new
   figures. The older figures have a flat look, while the newer figures
   feature gradient-filled notes and circular grab handles in the data
   paint. The following figure is older.

\begin{figure}[H]
   \centering 
%  \includegraphics[scale=1.00]{pattern-editor/pattern-edit-window.png}
   \includegraphics[scale=0.75]{pattern-editor/pattern-edit-window-2.png}
   \caption{External Pattern Editor Window}
   \label{fig:pattern_editor_window}
\end{figure}

   Recall that the grey dappled bars show the notes of the selected scale
   (the scale here does not reflect the scale used by the tune).
   The cyan dappled bars show the background pattern; here it is the
   rhythm cords.
   This figure shows recent new features, discussed later:

   \begin{itemize}
      \item For selected events, the orange data line is topped with a
         circular "grab handle" that can be used to change the amplitude of the
         data (e.g. velocity) being shown.
      \item Notes are, by default, shown with a gradient
         pattern for a textured appearance.
      \item The thin tooltip button above the pointing finger
         causes hovering over a note to show its number, time range,
         and velocity.
      \item New buttons at the right (including a button reserved for
         the future) to tweak some aspects of the pattern editor
         panels.
      \item Lines and numbers indicating MIDI values. The data pane
         has been expanded to allow easier dragging to the 0 and 127 
         extremes.
   \end{itemize}

\begin{figure}[H]
   \centering 
   \includegraphics[scale=1.00]{pattern-editor/pattern-edit-window-additions.png}
   \caption{External Pattern Editor Window Additions}
   \label{fig:pattern_editor_window_additions}
\end{figure}
  
   The \textbf{Pattern Editor} is complex, and we will discuss the external
   window only since its features are a superset of the \textbf{Editor} tab.
   ...
   Second, to add a note with the mouse the following options each turn on
   note painting:

   \begin{itemize}
      \item Press \textsl{and hold} the \textsl{right}
         mouse button (the pointer changes to a pointing finger),
         press the left mouse button while drawing.
   ...
   \texttt{x} to "eXit" from paint mode,
...
\end{figure}
   ...
   Note that there are recent additions to the data view
   (see \figureref{fig:pattern_editor_window_additions}).
   Refer to \sectionref{subsec:pattern_editor_right_buttons}.
...
\label{subsec:pattern_editor_first_row}
   ...
   time-signature.
   See \sectionref{subsubsec:pattern_editor_measures_ruler_time_bar}.
   ...
   However, if the "filter notes" button at the right of the
   data pane is checked, then only one note is inserted, and
   only notes in that chord are inserted.
...
\subsubsection{Pattern Editor / Second Row / Undo/Redo}
...
\subsubsection{Pattern Editor /  Second Row / Quantize Selection}
...
\subsubsection{Pattern Editor / Second Row / Tools Popup}
\label{subsubsec:pattern_editor_second_row_tools_popup}

   This button brings up a nested menu of tools for modifying selected
   events and notes. Shown below are selection options, and the
   small dialog to select a pitch range.
   It can also be brought up by the \texttt{Ctrl-T} keystroke.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{pattern-editor/rpn-option.png}
   \caption{Pattern Editor Tool Options}
   \label{fig:pattern_editor_window_tool_options}
\end{figure}

   \begin{enumber}
      \item \textbf{Insert macro at "L"}.
         Move the progress marker, if desired, to set the time-stamp,
         using a right-click in the upper half of the time pane.
         Then select a macro from the list to insert it at that time.
         The "footer" and "header" macros are always disabled, as they're
         not so useful on their own.
      ...
      This menu provides two note-selection options:
         \begin{itemize}
            \item \textbf{Select all}.
               Selects all notes in the pattern;
               \index{keys!ctrl-a} \texttt{Ctrl-A} will also select
               all of the events in the pattern editor.
            \item \textbf{Invert selection}.
               Inverts the selection of notes.
            \item \textbf{Select pitch range...}. 
               Brings up a small input dialog where the user can enter a single
               note value or a range of notes.
               The notes can be represented by either an integer (0-127)
               or a note name such as "C4".
               A single value selects an octave starting at that note.
               A range is notated as "32-64" or "A3-C\#4".
               The range is selected and can then be cut or moved.
               Also, an existing selection can be added to using
               this menu entry.
         \end{itemize}
      \item \textbf{Note timing/velocity...}.
         This menu offers four ways to tweak the timing of the selected notes:
         \begin{itemize}
            \item \textbf{Quantize}.
               \index{quantize}
      ...
      \item \textbf{LFO}.
         Allows for modulating of control values with a
         low-frequency oscillator.
         See \sectionref{subsec:pattern_editor_bottom}.
      \item \textbf{RPN/NRPN}.
         This entry brings up a dialog to set up the creation of a complete RPN
         or NRPN series of parameter change events, from the start to the data
         to the ending "reset" event.
         The events can be inserted into the pattern or
         be used to create a macro in the 'ctrl' file.
         The events can be saved to a file in text or binary formats.
         Pre-existing files (e.g. SysEx data) can be loaded.
         Arbitrary strings of MIDI bytes can also be created.
         See \sectionref{subsec:pattern_editor_rpn_nrpn_dialog}.
      \item \textbf{Pattern fix}.
         Allows for fixing up a new pattern
...
\subsubsection{Pattern Editor / Second Row / Follow Progress}
...
\subsubsection{Pattern Editor / Second Row / Snap Reset and Grid Snap}
\label{subsubsec:pattern_editor_second_row_grid_snap}
   ...
   Note: if the note does not snap close enough to where one clicks,
   simply reduce the snap value.

\subsubsection{Pattern Editor / Second Row / Note Length Reset/Note Length}
...
\subsubsection{Pattern Editor / Second Row / Zoom Reset/Zoom}
...
\subsubsection{Pattern Editor / Second Row / Key Reset/Key}
\label{subsubsec:pattern_editor_second_row_musical_key}
   ...
   and \textbf{B}.
   Changing the key
   shifts the marked note-row bars for the current scale or current
   chord selection.
   The keys pane at the left indicates the base notes
...
\subsubsection{Pattern Editor / Second Row / Scale Reset/Musical Scale}
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=1.00]{pattern-editor/estimated-scales.png}
   \caption{Estimating Scale(s) by Ctrl-K}
   \label{fig:pattern_editor_estimated_scales}
\end{figure}

   The method is not sophisticated... the notes are counted and are matched
   against all of the keys (C to B) and only the
   scales supported by \textsl{Seq66}.
   ...
   algorithm \cite{krumhansl}, could be used, but it is a bit too complex
   for our needs, which are basic.

\subsubsection{Pattern Editor / Second Row / Background Sequence}
...
\subsection{Pattern Editor / Measures Ruler (Time)}
...
\label{subsec:pattern_editor_piano_roll}
   ...
   sequencers, it is a bit different in feel.  A mouse with three
   buttons is helpful for editing, but not necessary.
   Buttons and keystrokes enhance the ease of editing.
   ...
   \index{keys!arrows}
   \textsl{If no notes are selected}, the arrow keys will move the piano roll
   ...
   passed to a sound generator or software synthesizer.
...
\label{subsubsec:pattern_editor_note_editing}
   ...
   The thin blank button
   next to the horizontal scroll bar toggles this mode, as its tooltip
   explains.
...
%              Also achievable via a \textbf{Middle Click Drag}.
      ...
      \item \index{pattern editor!deselect notes} \index{selection!deselect}
         ...
         To deselect all notes, click in an empty part of the piano roll.
         Or click on an unselected note (which becomes
   ...
   As shown in the following figure, "ghost notes" will appear as the mouse
   is moved, to aid in the placement of the notes. This feature is used
   for note movement and for pasting from the clipboard.

   \textbf{BUG}:
   The ghost notes are not shown when pasting into a different pattern.
   This happens because the code currently uses events already
   in the pattern.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.65]{pattern-editor/moving-notes.png}
   \caption{Note Movement and "Ghost Notes"}
   \label{fig:pattern_editor_moving_notes}
\end{figure}
...
\label{subsubsec:pattern_editor_events_pane_pattern_fix}
   ...
   It is also available from the \textbf{Tools} menu.
   See \sectionref{subsec:pattern_editor_pattern_fix}.
...
\label{subsec:pattern_editor_left_buttons}
   ...
   \begin{enumber}
      ...
      \item \textbf{Reserved for Expansion}
   \end{enumber}
...
\label{subsubsec:pattern_editor_left_buttons_drum_mode}
   ...
   Drum mode is a feature adopted from \textsl{Kepler34}.
   This button changes from normal note mode to drum note mode.
   In the drum mode, the notes are drawn as small red diamonds
   without any duration, at the beginning of the note location.
   This mode treats the events as having a minimal length.
   This can be confusing if the selected note length is large,
   so select a small note length.
   If the drum notes are not placed close enough to the desired
   location, reduce the snap size.
...
\subsubsection{Pattern Editor / Left Buttons / Blank Button}
\label{subsubsec:pattern_editor_left_buttons_blank_button}

   In order to get the height of the data pane to enlarge, after much
   head-banging, a button had to be added here for padding.
   It currently has no other purpose.

\subsection{Pattern Editor / Right Buttons}
\label{subsec:pattern_editor_right_buttons}

   This section describes some additional buttons as shown
   at the top of this chapter (see
   \figureref{fig:pattern_editor_window_additions}).

   \begin{enumber}
      \item \textbf{Show Pattern-Fix Dialog}
      \item \textbf{Toggle Scale/Chord Display}
      \item \textbf{Toggle Scale/Chord Note Filtering}
      \item \textbf{Toggle Hex Number Display}
      \item \textbf{Toggle Showing Grid Numbers Display}
      \item \textbf{Reserved for Expansion}
   \end{enumber}

\subsubsection{Pattern Editor / Right Buttons / Show Pattern-Fix Dialog}
\label{subsubsec:pattern_editor_right_buttons_show_pattern_fix}

   This button brings up the pattern-fix dialog.
   See \sectionref{subsubsection:patterns_fix_length_change}.

\subsubsection{Pattern Editor / Right Buttons / Toggle Scale/Chord Display}
\label{subsubsec:pattern_editor_right_buttons_scale_chord_display}

   A new feature of \textsl{Seq66} is the ability to show the note bars
   that \textsl{are not} in the selected chord.
   This feature is similar to the display of the selected musical scale
   (See \sectionref{subsubsec:pattern_editor_second_row_musical_scale}.)
   One difference is that the display of the scale has priority over the
   display of the chord.

   This button gives the user the option to show or not show the scale or
   chord.

\subsubsection{Pattern Editor / Right Buttons / Toggle Scale/Chord Note Filtering}
\label{subsubsec:pattern_editor_right_buttons_scale_chord_filtering}

   When this button is active (checked) and a scale has been selected,
   all notes painted into the piano roll are restricted to notes
   in the scale.
   Otherwise, any note value can be painted.

   When this button is active and a chord has been selected,
   the insertion of a full chord is disabled, but
   all notes painted into the piano roll are restricted to notes
   in the chord.
   Otherwise, painting a note paints a chord as per previous behavior.

\subsubsection{Pattern Editor / Right Buttons / Toggle Hex Number Display}
\label{subsubsec:pattern_editor_left_buttons_hex_number_display}

   When this button is active (checked), some of the values in the
   data pane will be displayed in hexadecimal format.
   This is most useful when controller numbers are being displayed.

\subsubsection{Pattern Editor / Right Buttons / Toggle Showing Grid Numbers Display}
\label{subsubsec:pattern_editor_left_buttons_grid_numbers}

   When this button is active (checked), the 5 level numbers
   are display at the left of the data pane.
   Toggle this button for a clearer view of numbers at the left.
...
\subsubsection{Pattern Editor / Data Pane / Pitchbend Events}
\label{subsubsec:pattern_editor_data_view_pitchbend_events}

   \textbf{Pitchbend}
   events are shown as a vertical lines that go above and below the midline.
   The only way to modify pitch bend events here is by dragging
   a line.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{pattern-editor/data-pane-pitchbend.png}
   \caption{Data Pane Pitch Bend}
   \label{fig:pattern_editor_data_pane_pitchbend}
\end{figure}

   Note that this figure has some new features not represented in the
   figures that follow.

   First, the data pane has been expanded slightly vertically so that the
   event grab handle circles can be used for dragging values near 0 or 127.

   Second, top and bottom horizontal lines show the limits of the data values.
   This includes numerical values shown at time 0. For the pitchbend value
   range, the values range from -2 to +2 semitones.
   \textsl{Seq66} currently does not know if an event has changed the
   pitchbend range for a given bus/channel.

   Third, a blank button at the left has been add to allow a greater
   height of the data pane.
   This kludge is needed because we don't completely grok the
   vagaries of Qt layouts.
...
\begin{figure}[H]
   ...
   \caption{Data Pane Circular Grab-Handles}
...
\end{figure}
   ...
   Data values can also be modified using the \textbf{LFO} 
   dialog (see below).
...
\end{figure}
   ...
   \begin{enumerate}
      \item Recording events via auto-step.
      \begin{enumerate}
         \item Turn on recording without the pattern playing.
         \item If desired, set the "L" marker to a start time other than 0.
         \item Activate the MIDI device's play or play some notes.
      \end{enumerate}
      \item Drawing events with the mouse.
      \begin{enumerate}
         \item Left-click-drag the mouse to enter notes.
         \item Note-drawing stops when at end of the pattern.
         \item Dragging back and forth will draw more notes.
      \end{enumerate}
   \end{enumerate}
...
\label{subsec:pattern_editor_pattern_fix}
   ...
   Applying a fix, then deciding to redo with a different fix, requires
   one to click the \textbf{Reset} before applying the next change.
   Once a change is made, the modify flag (asterisk) is shown,
...
\end{figure}
   ...
   When this dialog is used, the time-stamp of every event (except meta
   events at the beginning of the pattern) can be changed.
...
\label{subsubsection:patterns_fix_other_fixes}
   ...
   \begin{itemize}
      \item \textbf{Align left}.
         ...
         Meta events at the beginning (time 0) of the pattern
         are skipped when figuring out how much to shift.
...
\label{subsec:pattern_editor_lfo_panel}
   ...
   It can even modulate tempo and pitch-bend events.

   One recent change in this dialog is that selecting an option
   no longer resets (removes) the current changes.
   The \textbf{Reset} button must be clicked to do this.
...
% \includegraphics[scale=0.50]{pattern-editor/lfo-2.png}
  \includegraphics[scale=0.50]{pattern-editor/lfo-3.png}
...
\end{figure}
   ...
   \begin{enumber}
      \item \textbf{Use Measure (vs Length)}.
      ...
      \item \textbf{Multiply}.
         By default, the LFO waveform replaces the existing event
         "amplitude" values directly.
         With this option checked, the values of the existing events
         are \textsl{multiplied} by the waveform.
         One issue is that applying wave types multiple times tends
         to move all values toward zero.
...
%        For the sine-wave shown above, the range is 0 to 128 (actually 127).
      ...
      \item \textbf{Wave Type}.
         ...
         \begin{enumber}
            \item \textbf{None}.
               This setting is the initial setting.
               If selected after selecting on of the wave types below,
               all it does is reset the data, like the
               \textbf{Reset} button.
            \item \textbf{Sine}.
            ...
            \item \textbf{Exponential Rise}.
               ...
               Varying the \textbf{Phase} with the slider
               will move the peak of the curve left to right.
            \item \textbf{Exponential Fall}. Provides an exponential
               ramp modulation in the opposite direction.
            \item \textbf{DC Only}. Sets only the DC offset of the
               existing events. Does not work with the "Multiply" option.
               Problematic with pitch-bend events. One issue is that
               some precision in pitch-bend is lost as one changes the DC
               offset up and down, until clipping occurs.
         \end{enumber}
         Note that one might have to change one or more parameters
         slightly to see the effect of the new waveform.
      \item \textbf{Reset}.
         This button restores the initial pattern event data or
         the data stored by the \textbf{Lock} button.
         Useful when one applies modulation that one ultimately does not like.
      \item \textbf{Lock}.
         This button copies the current pattern changes to the backup
         data used by the \textbf{Reset} button.
...
\subsection{Pattern Editor / RPN/NRPN Dialog}
\label{subsec:pattern_editor_rpn_nrpn_dialog}

   This dialog provides a way to insert parameter changes much more
   easily than using the event editor.
   It inserts sequences of RPN or NRPN events
   based on the settings in the dialog.
   It can also create arbitrary data items and manage the
   \textsl{Seq66} MIDI macros.

   \textbf{Note:}

   This dialog cannot, at this time, look up MIDNAM data in order to populate
   the parameter names for NRPN.
   It does not handle the sequence of events needed to select a
   patch bank and a patch in that bank.
   The event editor can be used to (tediously) create these events.

\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{pattern-editor/rpnframe.png}
   \caption{RPN/NRPN Dialog}
   \label{fig:pattern_editor_rpn_nrpn_dialog}
\end{figure}

   The following text walks through the dialog elements.

   \begin{itemize}
      \item \textbf{Select Control}.
         The main purpose of this group is to select either an
         \textbf{RPN} or an
         \textbf{NPRN} event.
         One can also choose to enter a \textbf{Data} event alone, but
         there is not much reason to do that (yet).
         The \textbf{Data button increment/decrment} buttons
         are not yet implemented.
         The \textbf{Other (macros/arbitrary data)} allows typing or
         pasting events into the \textbf{Other's Data} field.
      \item \textbf{Select RPN}.
         On the other side, this group selects a specific RPN from
         the small set provided by the MIDI specification.
         These buttons are disabled if \textbf{Other} is
         selected.
      \item \textbf{Channel}.
         Provides the channel (1 to 16) to be applied to the RPN/NRPN
         controller events.
      \item \textbf{Bus}.
         Normally, the configured MIDI Control Out port is used for
         sending events. This dropdown allows the selection of another
         destination MIDI device.
      \item \textbf{Append data entry slider}.
         If checked (recommended) then, after the RPN/NPRN event
         bytes are created, the bytes need to set the parameter
         are appended.
         This buttons are disabled if \textbf{Other} is
         selected.
      \item \textbf{Append RPN reset}.
         If checked (recommended) then, add the RPN parameter reset
         event at the end.
         This buttons are disabled if \textbf{Other} is
         selected.
      \item \textbf{Use "fine" events}.
         If checked, also insert the "fine" version of the selected controller.
         The "fine" controllers are supported by General MIDI version 2.
         This buttons are disabled if \textbf{Other} is
         selected.
      \item \textbf{macro name}.
         Simply shows the selected/edited macro name. Not editable.
      \item \textbf{Timestamp (BBT)}.
         This text box displays, in B:B:T format by default,
         the timestamp at which the events will be inserted into the
         pattern.
      \item \textbf{BBT}.
         Clicking on this button toggles the timestamp display between
         B:B:T, H:M:S, and ticks (pulses).
      \item \textbf{Hex}.
         Clicking on this button toggles between showing the parameter numbers
         and parameter values in hexadecimal versus decimal format.
      \item \textbf{(N)RPN parameter no.}.
         This items displays the RPN number if RPN is selected, and that
         cannot be edited.
         If NRPN is selected, then the number of the desired
         parameter can be typed here.
         Note that parameter numbers are limited to 14-bits (0 to 16383).
         Unfortunately, at this time the user must know the NPRN parameter
         number.
      \item \textbf{Select macro}.
         Entering a short name into this editable combo-box enables the
         \textbf{Create Macro} button, or, if the macro exists,
         the \textbf{Modify Macro} button.
      \item \textbf{RPN/NRPN value}.
         Type the desired value of the current parameter here.
         It can be a single number (e.g. 1583 or 0x0600)
         or two numbers separated by a decimal point.
         An example is "12.50".
         For general parameters, "12" is the MSB value,
         and "50" is the LSB value.
         For pitchbend range, "12" is the semitone value, and
         "50" is the cents value.
         Note that parameter values are limited to 14-bits (0 to 16383).
      \item \textbf{Create Macro} or \textbf{Modify Macro}.
         If enabled and clicked, then the RPN events are added as a macro
         to the list of macros in the 'ctrl' configuration file,
         or modifies an existing macro.
         That file is marked to be saved at exit.
      \item \textbf{Delete Macro}.
         Deletes the currently selected macro from the active 'ctrl'
         file.
      \item \textbf{Insert Events}.
         Clicking this button adds the RPN/NRPN or "other" events to the
         pattern at the timestamp shown in the timestamp text box.
         This button is enabled only when the RPN/NRPN dialog is
         brought up via the pattern or event editor.
         Now, the sorting of MIDI events that \textsl{Seq66} does is
         a problem for an ordered sequence of events such as RPN and NRPN.
         To keep these multi-event sequences intact, the time
         of insert is incremented by one tick before inserting
         the next event.
      \item \textbf{Send Events}.
         Clicking this button sends the RPN/NRPN or "other" events to the
         MIDI Control Out (display) buss or the selected alternate buss.
      \item \textbf{Source}.
         When a file is loaded, or when the selected macro refers to
         data in a file, the full file-specification is displayed here.
      \item \textbf{Other's Data}.
         When \textbf{Other} is selected, this text editor field is enabled.
         One can type in the data bytes in either hexadecimal or decimal
         format. To create more than one event, separate the events
         with a pipe ("|"). Spaces \textsl{must} separate each token,
         including the pipe.
      \item \textbf{Event Info}.
         This text-box shows the generated data, always in hexadecimal.
         We might fix that later.
         If an error occurs, a message is shown in this box.
      \item \textbf{Bytes}.
         This box shows the number of total bytes in the event(s).
         Useful for indicating a long message.
      \item \textbf{Load}.
         This button opens a file dialog with filters for
         \texttt{*.macro} (textual macro file), \texttt{*.syx}
         (SysEx data file), and the usual "All Files" filter.
         Anything not a \texttt{*.macro} file consider a raw,
         binary file.
      \item \textbf{Save}.
         This button allows to save the data in text format or in
         a binary format.
      \item \textbf{Close}.
         All this button does is close the dialog.
   \end{itemize}

   There is a lot more that could be added to this macro editor.
   One thing is adding other controllers that have "coarse" and "fine"
   values, such as the controller and program events needed to
   select a patch bank and a particular patch.
   However, to be really useful, we would need incorporate the
   reading of MIDINAM files.
   This is something we are only thinking about at present.
   See \url{https://github.com/ahlstromcj/midiname}.
